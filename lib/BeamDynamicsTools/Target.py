...
class Target:
    def __init__(self, NORM, TAN, INC, BFieldTF, BFieldVF, RT, Xpol, Rdet=[1.3075, -0.2457, -0.05900], AxisDet=[1.0, 0.0, 0.0]):
        ...
        if dR[0] > 0:
            self.Theta = arccos(
                dot(-1.0 * RCenter / norm(RCenter), dR / norm(dR)))
        else:
            self.Theta = -1.0 * \
        ...
        # Basis Vectors
        e3 = array(NORM) / norm(NORM)
        self.NormalV = e3  # self.e3=e3
        e2 = array(TAN) / norm(TAN)
...
# Calculate Angular parameters and vectors
        ...
        NormXY = array([self.NormalV[0], self.NormalV[1]])
        NormXY / norm(NormXY)
        IncXY = -1.0 * array([self.IncidentV[0], self.IncidentV[1]])
        IncXY / norm(IncXY)
...
# Project transverse ellipse on target
    ...
    def PlotProjection(self, Type='Centered'):
        ...
        if Type == 'ThetaPhi':
            ...
            XLAB = r'Toroidal Position $R\cdot\phi$ [cm]'
            xscale = 100.0
            YLAB = r'Polidal Position $r\cdot\theta$ [cm]'
            yscale = 100.0
